...
def generate_gaussian_weight(bbox,size_2d,sigma_scale):
    (h,w) = size_2d
    (sw,sh) = ((bbox[2]-bbox[0]) / 2*size_2d[1]*sigma_scale, (bbox[3]-bbox[1]) / 2*size_2d[0]*sigma_scale)
    centers_int = [((bbox[2]+bbox[0]) / 2 *size_2d[1]).round(),((bbox[3]+bbox[1]) / 2*size_2d[0]).round()]


    h_range = torch.arange(0,h).unsqueeze(-1).to(bbox.device)
    h_grid = h_range.repeat(1,w)
    w_range = torch.arange(0,w).unsqueeze(-2).to(bbox.device)
    w_grid = w_range.repeat(h,1)
    gs_weight = torch.exp(- 0.5*(w_grid - centers_int[0])**2/sw**2 - 0.5*(h_grid - centers_int[1])**2/sh**2)
    
    return gs_weight
    
def generate_heatmap(bboxes, heatmap_size=30, box_format='corner'):
    """
    ...
    return torch.cat(gaussian_maps,dim=0) #(N,4)
