...
from utils.image import box2label
import numpy as np
import torch
...
class VOSTrain(Dataset):
    ...
    def get_ref_index(self,
        ...
        for _ in range(max_try):
            ...
            if ref_index in bad_indices:
                continue
            if self.box_label:
                non_zero = lablist[ref_index][-2] * lablist[ref_index][-1]
            else:
                ...
                non_zero = len(xs)
            if non_zero > min_fg_pixels and is_consistent:
                break
    ...
    def get_image_label(self, seqname, imagelist, lablist, index):
        ...
        if self.rgb:
            image = image[:, :, [2, 1, 0]]
        if self.box_label:
            label = box2label(lablist[index],image.shape[:2])
        else:
            label = Image.open(
    ...
    def sample_sequence(self, idx):
        ...
        # ensure img/label sizes the same
        (h,w) = ref_image.shape[:2]
        if prev_image.shape[:2] != (h,w):
            prev_image = cv2.resize(prev_image,(w,h),interpolation=cv2.INTER_CUBIC)
        for i in range(len(curr_images)):
            if curr_images[i].shape[:2]!=(h,w):
                curr_images[i] = cv2.resize(curr_images[i],(w,h),interpolation=cv2.INTER_CUBIC)
        if ref_label.shape != (h,w):
            ref_label = cv2.resize(ref_label,(w,h),interpolation=cv2.INTER_NEAREST)
        if prev_label.shape != (h,w):
            prev_label = cv2.resize(prev_label,(w,h),interpolation=cv2.INTER_NEAREST)
        for i in range(len(curr_labels)):
            if curr_labels[i].shape[:2]!=(h,w):
                curr_labels[i] = cv2.resize(curr_labels[i],(w,h),interpolation=cv2.INTER_NEAREST)
...
class LASOT_Train(VOSTrain):
    def __init__(self,
        ...
        # image_root = os.path.join(root, 'images')
        image_root = os.path.join(root, 'JPEGImages')
        label_root = os.path.join(root, 'BoxAnnotations' if box_label else 'Annotations')
        seq_names = list(np.sort(os.listdir(os.path.join(image_root))))
        ...
        for seq_name in seq_names:
            ...
            if box_label:
                with open(os.path.join(label_root,seq_name+'.txt')) as f:
                    labels = f.readlines()
                    labels = [list(map(float,x.strip('\n').split(','))) for x in labels]
            else:
