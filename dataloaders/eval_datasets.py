...
class VOTTest(Dataset):
    def __init__(self,
        ...
        self.obj_nums = [1]*self.num_frame
        self.obj_indices = [[0,1]]*self.num_frame
    ...
    def read_label_box2mask(self, label_path, size_2d, squeeze_idx=None):
        with open(label_path,'r') as f:
            line = f.readline()
        box = list(map(int,map(float,line.strip('\n').split(','))))
        if len(box)!=4:
            raise ValueError("invalid box annotation")
        ...
        # x1,y1,w,h format by default
        label = np.zeros(size_2d,dtype=np.float32)
        label[box[1]:box[1]+box[3]+1,box[0]:box[0]+box[2]+1] = 1
    ...
    def __getitem__(self, idx):
        ...
        if idx==0:
            current_label = self.read_label_box2mask(self.labels[0], (height,width), obj_idx)
            sample = {
                'current_img': current_img,
                'current_label': current_label
            }
...
class LaSOT_Test(object):
    def __init__(self,
                 root='./datasets/LaSOTTest',
                 split='test',
                 transform=None,
                 rgb=True,
                 result_root=None):
        ...
        seqs_path = glob(self.image_root+'/*')
        self.seqs = [s.split('/')[-1] for s in seqs_path if '.' not in s]
    ...
    def __getitem__(self, idx):
        seq_name = self.seqs[idx]
        images = []
        labels = [os.path.join(self.label_root,seq_name,'groundtruth.txt')]
        images = sorted(glob(os.path.join(self.image_root,seq_name,'img')+'/*.jpg'),
        ...
        seq_dataset = VOTTest(self.image_root,
                              self.label_root,
                              seq_name,
                              images,
                              labels,
...
class TrackingNet_Test(object):
    def __init__(self,
                 root='./datasets/TrackingNetTest',
                 split='test',
                 transform=None,
                 rgb=True,
                 result_root=None):
        ...
        self.image_root = os.path.join(root,'JPEGImages')
        self.label_root = os.path.join(root, 'BoxAnnotations')
    ...
    def __getitem__(self, idx):
        ...
        images = []
        labels = []
        images = sorted(glob(os.path.join(self.image_root,seq_name)+'/*.jpg'),
                        key=lambda x:int(x.split('/')[-1].split('.')[0]))
        labels = [os.path.join(self.label_root,seq_name+'.txt')]
        # print(images,labels)
        try:
            if not os.path.exists(os.path.join(self.result_root,
                                                seq_name)):
...
class GOT10K_Test(object):
    def __init__(self,
                 root='./datasets/GOT10KTest',
                 split='test',
                 transform=None,
                 rgb=True,
                 result_root=None):
