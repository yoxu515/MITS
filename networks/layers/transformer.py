...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
                 ...
                 norm_layer=None):
        super().__init__()
        if not isinstance(patch_size,abc.Iterable):
...
class BoxTransformer(nn.Module):
    def __init__(self,
                 ...
                 dual_cross=False,
                 droppath=0.1,
                 droppath_cross=False,
                 ):

        super().__init__()
        self.num_layers = num_layers

        block = BoxTransformerBlock
        layers = []
        for i in range(num_layers):
            layers.append(
                block(d_model, self_nhead, att_nhead, dim_feedforward,
                      droppath, droppath_cross,dual_cross))
        self.layers = nn.ModuleList(layers)


    def forward(self,img_emb,boxes_emb):

        for idx, layer in enumerate(self.layers):
            img_emb, boxes_emb = layer(img_emb,boxes_emb)
...
class BoxTransformerBlock(nn.Module):
    def __init__(self,
        ...
        self.d_model = d_model
        self.att_nhead = att_nhead
        self.dual_cross = dual_cross
        self.drop_cross = drop_cross

        # self-attention for boxes
        self.norm_boxes = _get_norm(d_model)
        self.self_attn_boxes = MultiheadAttention(d_model, self_nhead)

        # self-attention for image
        self.norm_img = _get_norm(d_model)
        self.self_attn_img = MultiheadAttention(d_model,self_nhead)
        
        # cross-attention
        self.norm_cross_img = _get_norm(d_model)
        self.norm_cross_boxes = _get_norm(d_model)
        self.cross_q_img = nn.Linear(d_model,d_model)
        self.cross_kv_boxes = nn.Linear(d_model,2 * d_model)
        self.cross_attn_img = MultiheadAttention(d_model, att_nhead, use_linear=False)
        if self.dual_cross:
            self.cross_q_boxes = nn.Linear(d_model,d_model)
            self.cross_kv_img = nn.Linear(d_model,2 * d_model)
            self.cross_attn_boxes = MultiheadAttention(d_model, att_nhead, use_linear=False)

        # feed-forward for image
        self.norm_ffn_img = _get_norm(d_model)
        self.linear1_img = nn.Linear(d_model, dim_feedforward)
        self.activation_img = nn.GELU()
        self.linear2_img = nn.Linear(dim_feedforward, d_model)

        # feed-forward for boxes
        self.norm_ffn_boxes = _get_norm(d_model)
        self.linear1_boxes = nn.Linear(d_model, dim_feedforward)
        self.activation_boxes = nn.GELU()
        self.linear2_boxes = nn.Linear(dim_feedforward, d_model)
    ...
    def forward(self, img_emb, boxes_emb):
        ...
        # self-attention for boxes
        _boxes_emb = self.norm_boxes(boxes_emb)
        q = k = v = _boxes_emb
        boxes_self = self.self_attn_boxes(q,k,v)[0]
        boxes_emb = boxes_emb + self.droppath(boxes_self)
        
        # self-attention for image
        _img_emb = self.norm_img(img_emb)
        q = k = v = _img_emb
        img_self = self.self_attn_img(q, k, v)[0]
        img_emb = img_emb + self.droppath(img_self)

        # cross atention
        _img_emb = self.norm_cross_img(img_emb)
        _boxes_emb = self.norm_cross_boxes(boxes_emb)

        q = self.cross_q_img(_img_emb)
        kv = self.cross_kv_boxes(_boxes_emb)
        k,v = torch.split(kv,self.d_model,dim=-1)
        img_cross = self.cross_attn_img(q,k,v)[0]
        if self.drop_cross:
            img_emb = img_emb + self.droppath(img_cross)
        else:
            img_emb = img_emb + img_cross

        if self.dual_cross:
            q = self.cross_q_boxes(_boxes_emb)
            kv = self.cross_kv_img(_img_emb)
            k,v = torch.split(kv,self.d_model,dim=-1)
            boxes_cross = self.cross_attn_boxes(q,k,v)[0]
            if self.drop_cross:
                boxes_emb = boxes_emb + self.droppath(boxes_cross)
            else:
                boxes_emb = boxes_emb + boxes_cross

        # Feed-forward
        _img_emb = self.norm_ffn_img(img_emb)
        img_ffn = self.linear2_img(self.activation_img(self.linear1_img(_img_emb)))
        img_emb = img_emb + self.droppath(img_ffn)

        _boxes_emb = self.norm_ffn_boxes(boxes_emb)
        boxes_ffn = self.linear2_boxes(self.activation_boxes(self.linear1_boxes(_boxes_emb)))
        boxes_emb = boxes_emb + self.droppath(boxes_ffn)

        return img_emb,boxes_emb
...
class TransformerHeadBlock(nn.Module):
    def __init__(self,
                 d_model,
                 att_nhead,
                 dim_feedforward=1024,
        ...
        # self-attention
        self.norm1 = _get_norm(d_model)
        self.self_attn = MultiheadAttention(d_model, att_nhead)
        
        # feed-forward
        self.norm_ffn = _get_norm(d_model)
        self.linear1 = nn.Linear(d_model, dim_feedforward)
    ...
    def forward(self, emb):
        ...
        _emb = self.norm1(emb)
        q = k = v = _emb
        _emb_self = self.self_attn(q,k,v)[0]
        emb = emb + self.droppath(_emb_self)
        
        # Feed-forward
        _emb = self.norm_ffn(emb)
        _emb_ffn = self.linear2(self.activation(self.linear1(_emb)))
        emb = emb + self.droppath(_emb_ffn)
