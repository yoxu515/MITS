...
from dataloaders.train_datasets import DAVIS2017_Train, YOUTUBEVOS_Train, StaticTrain, TEST, BL30K_Train, LASOT_Train, GOT10K_Train
...
from networks.layers.vision_transformer import _load_weights
...
class Trainer(object):
    def __init__(self, rank, cfg, enable_amp=True):
        ...
        if cfg.TRAIN_BOX_ID_ENCODER_ONLY:
            for param in self.model.named_parameters():
                if 'box_id_encoder' not in param[0]:
                    param[1].requires_grad = False
        if cfg.TRAIN_BOXT_ONLY:
            for param in self.model.named_parameters():
                if 'BoxT' in param[0]:
                    pass
                else:
    ...
    def process_pretrained_model(self):
        ...
        if cfg.TRAIN_RESUME:
            ...
            if cfg.MODEL_BOXT_PRETRAIN != '':
                _load_weights(self.model.BoxT,cfg.MODEL_BOXT_PRETRAIN)
    ...
    def prepare_dataset(self):
        ...
        if 'lasot' in cfg.DATASETS:
            train_lasot_dataset = LASOT_Train(
                ...
                enable_prev_frame=self.enable_prev_frame,
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM,
                box_label=cfg.DATA_LASOT_BOX_LABEL
            )
            train_datasets.append(train_lasot_dataset)
        
        if 'got10k' in cfg.DATASETS:
            train_lasot_dataset = GOT10K_Train(
                root=cfg.DIR_GOT10K,
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                ...
                enable_prev_frame=self.enable_prev_frame,
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM,
                box_label=cfg.DATA_GOT10K_BOX_LABEL
            )
            train_datasets.append(train_lasot_dataset)
    ...
    def process_log(self, ref_imgs, prev_imgs, curr_imgs, ref_labels,
        ...
        if cfg.TRAIN_IMG_LOG or cfg.TRAIN_TBLOG:
            ...
            if cfg.TRAIN_TBLOG:
                ...
                if step % cfg.TRAIN_TBLOG_IMG_STEP == 0:
                    for key in boards['image'].keys():
                        tmp = boards['image'][key].clone().detach().cpu().numpy()
                        self.tblogger.add_image('S{}/' + key, tmp, step)
                for key in boards['scalar'].keys():
                    if isinstance(boards['scalar'][key],torch.Tensor):
                        tmp = boards['scalar'][key].clone().detach().cpu().numpy()
                    else:
